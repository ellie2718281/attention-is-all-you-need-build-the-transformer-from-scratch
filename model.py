...
def pad_id_sequence(ids, max_len, pad_id):
    ...
    if len_ids>max_len:
        return ids[:max_len]
    elif len_ids<max_len:
        return ids+[pad_id]*(max_len-len_ids)
    else:
        return ids
...
def combine_padding_and_causal_masks(padding_mask, causal_mask):
    ...
    return padding_mask & causal_mask #causal mask avoid model to look at future...
...
def split_qkv_into_heads(q, k, v, num_heads):
    ...
    q_trans = transpose_heads_before_sequence(q_split)
    k_trans = transpose_heads_before_sequence(k_split)
    v_trans = transpose_heads_before_sequence(v_split)
    return (q_trans, k_trans, v_trans)
...
def apply_residual_add_and_norm(residual_input, sublayer_output, gamma, beta, eps=1e-5):
    ...
    combine = residual_input + sublayer_output
    return normalize_and_scale_with_gamma_beta(combine, gamma, beta, eps)
...
def encoder_layer_self_attention_sublayer(x, w_q, w_k, w_v, w_o, gamma, beta, num_heads, src_mask):
    ...
    attention_out = assemble_multi_head_attention_forward(x, x, x, w_q, w_k, w_v, w_o, num_heads, src_mask)
    return apply_residual_add_and_norm(x, attention_out, gamma, beta)
...
def decoder_layer_masked_self_attention_sublayer(y, w_q, w_k, w_v, w_o, gamma, beta, num_heads, tgt_mask):
    ...
    attention_out = assemble_multi_head_attention_forward(
        y, y, y, w_q, w_k, w_v, w_o, num_heads, tgt_mask
    )
    return apply_residual_add_and_norm(y, attention_out, gamma, beta)
...
def run_transformer_forward(src_ids, tgt_ids, model_params, num_heads, pad_id):
    ...
    # 4. Build source padding mask
    src_mask = build_padding_mask(src_ids, pad_id)

    # 5. Build target padding + causal mask
    ...
    # 8. Project decoder output to vocabulary logits
    logits = apply_final_output_projection(
        decoder_output,
        model_params["output_projection"]
    )

    # 9. Convert logits to log probabilities
...
def init_embedding_and_projection_parameters(vocab_size, d_model, tie_weights=True):
    ...
    src_embedding = torch.nn.Parameter(
        torch.randn(vocab_size, d_model) * 0.02
    )

    tgt_embedding = torch.nn.Parameter(
        torch.randn(vocab_size, d_model) * 0.02
    )
...
def collect_model_parameters_into_list(
    ...
    # walk encoder, decoder, and embedding dicts
    # and return a flat deduped list of tensors
